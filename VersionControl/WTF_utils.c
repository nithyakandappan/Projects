...
void LLpush(struct LLnode ** head, char * dpath, char * dvNum,char * dhash);
...
struct LLnode * LLsearch(struct LLnode * head, char * word) { //returns bitCode or -1 if the word is not there
...
{
        struct LLnode* new=(struct LLnode *)malloc(1*sizeof(struct LLnode));
        new->path=(char *) malloc(strlen(dpath));
        strcpy(new->path, dpath);
        new->vNum=(char *) malloc(strlen(dvNum));
        strcpy(new->vNum, dvNum);
        new->hash=(char *) malloc(strlen(dhash));
        strcpy(new->hash, dhash);
        new->next=*head;
        *head = new;
}
...
void  LLfromManifest(struct LLnode **head,char *file){
        ...
        token = strtok_r(file,delim1,&saveptr1);
